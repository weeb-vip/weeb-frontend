<script lang="ts">
  ...
  const sectionClass = $derived(userSectionClass(isMobile));
  const itemClass = $derived(menuItemClass(isMobile));
  const logoutClass = $derived(logoutButtonClass(isMobile));
