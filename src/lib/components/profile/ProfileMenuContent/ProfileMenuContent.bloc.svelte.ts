...
import { logout } from '$lib/services/query-options';
...
/*
  The menu is drawn twice -- in the desktop dropdown and in the mobile drawer --
  and the two differ only in padding, type scale and hover treatment. The three
  class lists that encode that live here rather than as ternaries through the
  markup, so the view reads as a list of links.
*/

export function userSectionClass(isMobile: boolean): string {
  return isMobile
...
export function menuItemClass(isMobile: boolean): string {
  return isMobile
...
export function logoutButtonClass(isMobile: boolean): string {
  return isMobile
