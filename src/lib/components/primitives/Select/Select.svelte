...
<script lang="ts">
  ...
  const label = $derived(triggerLabel(options, value, placeholder));
  ...
  function position() {
    ...
    menuMinWidth = placement.minWidth;
  ...
  async function openMenu() {
    ...
    activeIndex = optionIndexOf(options, value);
    position();
    await tick();
  ...
  function move(delta: number) {
    ...
    activeIndex = movedIndex(activeIndex, delta, options.length);
    scrollActiveIntoView();
  }
  ...
  >
    {#each options as option, i (option.value)}
      <button
        ...
        class:wv-select-option--selected={isSelectedOption(option, value)}
        data-index={i}
        role="option"
