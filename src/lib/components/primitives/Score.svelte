<script lang="ts">
  ...
  const text = $derived(typeof value === 'number' ? value.toFixed(1) : (value ?? ''));
  const hasScore = $derived(value !== null && value !== undefined && value !== '');
</script>

{#if hasScore || variant === 'inline'}
  <span class="score score--{variant} {className}" class:no-score={!hasScore}>
    {#if hasScore}
