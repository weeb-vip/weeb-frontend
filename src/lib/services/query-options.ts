/**
 * TanStack Query *option factories* over the GraphQL layer.
 *
 * Every export here is a plain function returning a `{ queryKey, queryFn }` (or
 * `{ mutationFn }`) object plus the auth/refresh plumbing those need. It is
 * framework-agnostic: no runes, no `createQuery`, so route `load` functions,
 * workers and the SSR path can all call it.
 *
 * The layer above is `./query-hooks.ts`, which wraps these in Svelte Query
 * hooks; the layer below is `./api/graphql/queries.ts`, which holds the raw
