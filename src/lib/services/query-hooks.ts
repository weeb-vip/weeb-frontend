/**
 * Svelte Query hooks -- the component-facing half of the data layer.
 *
 * `useLogin`, `useUser`, `useHomePageData` and friends wrap the option
 * factories in `./query-options.ts` with `createQuery`/`createMutation`, and
 * add the cross-cutting behaviour a component wants: cache invalidation, the
 * logged-in store, analytics identification, token refresh scheduling.
 *
 * Anything without a component to render into (a route `load`, a worker, SSR)
 * should call `./query-options.ts` directly instead.
 */
...
import { analytics, identifyUser } from "$lib/utils/analytics";
